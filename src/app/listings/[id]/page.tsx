...
const STATUS_LABEL: Record<string, { label: string; className: string }> = {
  ...
  partial: { label: 'Partially Available', className: 'bg-yellow-50 text-yellow-700' },
  booked:  { label: 'Booked',              className: 'bg-red-50 text-red-600' },
        ...
        <div className="mb-6">
          <div className="relative flex h-72 items-center justify-center overflow-hidden rounded-2xl bg-black">
            {room.images?.[activeImage] ? (
              <Image src={room.images[activeImage]} alt={room.title} fill className="object-cover" />
            ) : (
          ...
          {room.images && room.images.length > 1 && (
            <div className="mt-3 grid grid-cols-4 gap-2">
              {room.images.map((image, index) => (
                ...
                >
                  <Image src={image} alt={`Room image ${index + 1}`} fill className="object-cover" />
                </button>
              ))}
            </div>
          )}
        ...
        <div className="flex flex-col gap-6 md:flex-row">
          ...
          <div className="flex-1">
            <div className="mb-2 flex items-start justify-between gap-4">
              <h1 className="text-2xl font-semibold text-gray-900">{room.title}</h1>
            ...
            <div className="mb-5 flex flex-wrap items-end gap-6">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-gray-700">{room.total_seats}</p>
              </div>
              <div>
                <p className="text-xs text-gray-400">Available Seats</p>
                <p className="text-lg font-semibold text-blue-600">{room.available_seats}</p>
              </div>
            </div>
