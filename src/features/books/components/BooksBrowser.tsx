...
import { mapListingToBookCard } from '@/features/books/lib/mapListingToBookCard'
import type { BookListing } from '@/features/books/types'

export default function BooksBrowser({ listings }: { listings: BookListing[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return listings.filter((listing) => {
      ...
      const haystack = [listing.title, details?.author, details?.course_code, details?.department]
        .filter(Boolean)
        .join(' ')
        .toLowerCase()
  ...
  return (
    <main className="page-shell py-8">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Used Books Marketplace</h1>
          <p className="mt-1 text-sm text-gray-400">Browse textbooks and reference materials from students near your campus.</p>
        </div>
        <Link
          href="/post-book"
        ...
        >
          <Plus className="h-4 w-4" aria-hidden /> Post a Book
        </Link>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-gray-200 py-16 text-center dark:border-gray-700">
          <p className="text-sm text-gray-400">
            {listings.length === 0 ? 'No books posted yet — be the first!' : 'No books match your search.'}
          </p>
        </div>
      ) : (
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
          {filtered.map((listing) => (
            <ContentCard key={listing.id} {...mapListingToBookCard(listing)} />
          ))}
        </div>
      )}
    </main>
