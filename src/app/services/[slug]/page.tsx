...
import { SERVICES, getDiscoveryItem } from '@/features/discovery/data'
...
export async function generateMetadata({
  ...
  const item = getDiscoveryItem('service', slug)
  return {
    title: item?.title[locale === 'bn' ? 'bn' : 'en'] ?? 'Service preview',
...
export default async function ServiceDetailRoute({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <DiscoveryDetailPage kind="service" slug={slug} />
}
