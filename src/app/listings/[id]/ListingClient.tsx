...
export default function ListingClient({ id }: { id: string }) {
  ...
  const availableDateLabel = room.available_from
  ...
  const availableText = room.available_from
    ? new Date(room.available_from) <= new Date()
      ? `Available now · since ${availableDateLabel}`
      : `Available from ${availableDateLabel}`
    : null
  ...
  return (
    <>
      <main className="mx-auto max-w-4xl px-4 py-6">
        ...
        {/* IMAGES */}
        <div className="mb-6">
          <div className="relative h-80 overflow-hidden rounded-2xl bg-gray-900 sm:h-[26rem]">
            {room.images && room.images.length > 0 ? (
              <>
                {room.images.map((src, index) => (
                  ...
                  >
                    <Image src={src} alt="" aria-hidden fill className="scale-110 object-cover opacity-50 blur-2xl" />
                    <Image src={src} alt={room.title} fill className="object-contain" />
                  </div>
                ))}
        ...
        <div className="flex flex-col gap-6 md:flex-row">
          ...
          <div className="flex-1">
            <div className="mb-3">
              <span className={`float-right ml-3 mb-1 shrink-0 rounded-full px-3 py-1 text-xs font-semibold ${status.className}`}>
                {status.label}
              </span>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{room.title}</h1>
            </div>

            <div className="mb-2 flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400">
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-col gap-1.5">
                  {room.landmarks.map((lm, i) => (
                    <div key={i} className="flex items-center gap-1.5 rounded-xl border border-teal-100 bg-teal-50/70 px-3 py-2.5 text-sm dark:border-teal-800 dark:bg-teal-900/10">
                      <MapPin className="h-3.5 w-3.5 shrink-0 text-teal-500" />
                      <span className="font-medium text-gray-700 dark:text-gray-300">{lm.name}</span>
                      {lm.time && <span className="text-xs font-semibold text-teal-600 dark:text-teal-400">– {lm.time}</span>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {(room.advance_deposit || room.rent_inclusive || room.university_priority) && (
              <div className="mb-5 flex flex-col gap-2.5 rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4 dark:border-indigo-800 dark:bg-indigo-900/10">
                <p className="mb-1 flex items-center gap-1.5 text-xs font-semibold text-indigo-700 dark:text-indigo-400">
                  <Tag className="h-3.5 w-3.5" /> Terms
                </p>
                {room.advance_deposit && (
                  <div className="flex justify-between gap-3 text-sm">
                    <span className="shrink-0 text-gray-500 dark:text-gray-400">Advance Deposit</span>
                    <span className="text-right font-semibold text-gray-700 dark:text-gray-200">{room.advance_deposit}</span>
                  </div>
                )}
                {room.rent_inclusive && (
                  <p className="text-sm font-medium text-teal-700 dark:text-teal-400">✓ Utilities included in rent</p>
                )}
                {room.university_priority && (
                  <div className="flex justify-between gap-3 text-sm">
                    <span className="shrink-0 text-gray-500 dark:text-gray-400">Preferred tenants</span>
                    <span className="text-right font-medium text-gray-700 dark:text-gray-200">{room.university_priority}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {room.description && (
              <div className="mb-5 rounded-2xl border border-gray-200 bg-white px-4 py-4 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            {availableText && (
              <div className="mb-5 flex items-center gap-2 rounded-xl border border-teal-100 bg-teal-50 px-4 py-3 text-sm font-semibold text-teal-700 dark:border-teal-800 dark:bg-teal-900/20 dark:text-teal-400">
                <Calendar className="h-4 w-4 shrink-0" />{availableText}
              </div>
            )}
          </div>
